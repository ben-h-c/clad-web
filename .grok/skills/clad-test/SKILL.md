...
- Midterms overlay: `npm run check:races` when touching `liveOverlay.ts`, `raceCalls.ts`, or the auditor/caller. After staging, `/bracket/` names should match current nominees without a `races.ts` edit. Winner calls require AP or two majors and a general/special vote day.
