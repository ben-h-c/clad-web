...
Staging may run **layout experiments** (`html[data-skin]`) that change structure, type, density, and chrome — not just color. **Cover is the production home lead** (2026-08-16): first Breaking story is a full-bleed plate — phone `min-height: 16.25rem` (room for grade + 4-line headline; not a tight 16:9), `min-height: max(15rem, 40svh)` from 700px up — headline + grade only (no date line), Today tape in even **1.45rem** paper (clear titles `margin-top`; match below); other strips stay carousels. Cover lead mute-loops a same-origin native `<video>` of that story’s own clip. **Never put a phone/Shorts video on the Cover** — only full-frame landscape. Shorts stay in the Breaking strip. Website Cover is a 16:9 card in the page column (not a 100vw slit, not letterboxed). Do not use a YouTube iframe on the plate. Only the text-readability gradient behind the headline. Tap opens the article. `prefers-reduced-motion` keeps the still. Staging Skin row is hidden until a new experiment exists (`STAGE_SKINS` empty). Retired Packed–Wire / `?skin=cover` fall back to this Current. Recolor-only skins have already been rejected.
...
- **Home lead stack:** Today → Breaking → Front Page sit tighter than later sections (Current / Soft Neutral only; adjacent-sibling CSS). Do not collapse Discover / Good News / People unless asked. **Breaking and Front Page never share an article** — Breaking keeps it; Front Page yields (including members of a Breaking group).
...
- **iPhone App Store card:** top-of-page Soft Neutral card (`IosAppBanner`) on iPhone browsers only. App Store + ×. Hidden in-app. Do not restack Apple’s smart banner on iPhone.
