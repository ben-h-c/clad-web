...
/** Resolve a Wikipedia title to a validated Commons thumbnail (free license only). */
async function commonsThumbForWikiTitle(title) {
  const t = String(title || "").trim();
  if (!t) return null;
  const enc = encodeURIComponent(t.replace(/ /g, "_"));

  // 1) REST summary (fast) — keep API width; validate + try safe alternates
  try {
    ...
    if (r.ok) {
      ...
      if (j?.type !== "disambiguation") {
        ...
        const valid = await firstValidCommonsUrl(src, imgOpts);
  ...
  // Request 500px (known-good band); still validate before return.
  try {
...
async function resolveHistoryImage(raw) {
  // Keep a stored URL only when it still validates (dead thumbs re-resolve).
  if (raw?.imageUrl) {
    const valid = await firstValidCommonsUrl(String(raw.imageUrl), imgOpts);
    if (valid) return valid;
  }
...
/** True when any stored imageUrl is missing or fails validation. */
async function packNeedsThumbRepair(items) {
  for (const i of items || []) {
    if (!i?.imageUrl) return true;
    const cleaned = cleanCommonsUrl(String(i.imageUrl));
    if (!cleaned) return true;
    if (!(await validateImageUrl(cleaned, imgOpts))) return true;
  }
  return false;
}
...
export async function runTodayInHistory(agent) {
  ...
  if (existing.ok && !force) {
    ...
    if (
      ...
      const needsThumbs = await packNeedsThumbRepair(store.items);
      if (!missingVideo && !needsThumbs) {
        return {
          ok: true,
      ...
      // Same desk day but missing/dead thumbs and/or embeds: re-attach media, keep copy.
