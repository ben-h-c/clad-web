...
- **Masthead Menu:** four chapters first — Read / Coverage / Play / About as independent folds (`masthead__menu-fold`). First open shows only those names (plus Account). Tap a chapter to reveal its links. Closing Menu resets folds. Current page is a thin teal left rule; the chapter that contains it gets the same mark on its name. Account is a quiet last row (never the display name). Search stays the magnifier. Guests keep **See grades free** above the directory. Footer stays Product / Explore / Org.
