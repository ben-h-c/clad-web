...
- **Home “Today” title bar:** compact horizontal title links (`.home-features--titles` / `.today-title-bar`), not media heroes. Manual pan-x, mid-dot separators, optional short kickers for desk/daybook only, right-edge fade. People in the news stays media heroes (`HomeFeatureHighlight` default). Breaking is the first visual focus after the thin bar.
