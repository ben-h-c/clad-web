...
**Desk publish:** Worker `GITHUB_TOKEN` (fine-grained PAT, Contents R/W on `ben-h-c/clad-web`, 366-day expiry). Agent drafts **auto-publish** in production (`AUTO_APPROVE_DRAFTS`). A 401 means the PAT died — rotate with `node scripts/rotate-github-token.mjs` (no code deploy). `/admin/health` live-pings GitHub.
