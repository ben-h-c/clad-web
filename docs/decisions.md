...
**Status:** accepted  
**Context:** Ben: switch CladFacts back to full mode from economy and push to prod.  
**Decision:** Run **full**. `XAI_ECONOMY=full` on the Mac runner (`runner/.env` + PM2 restart) and the production Worker (`wrangler.jsonc` vars). Staging stays economy / spend-dark. `XAI_ECONOMY=economy` is the throttle.  
**Consequences:** Full YouTube draft caps, vision on publish, reasoning curators, X search on sentiment, cron without `minHoursBetweenRuns`. Revert with `XAI_ECONOMY=economy` + runner restart + Worker deploy.

## 2026-08-30 — xAI spend mode is economy

**Status:** accepted  
**Context:** Ben: put CladFacts back in economy mode and push to prod. Live had been full since 2026-08-29.  
**Decision:** Run **economy**. `XAI_ECONOMY=economy` on the Mac runner (`runner/.env` + PM2 restart) and the production Worker (`wrangler.jsonc` vars). Staging stays economy / spend-dark. `XAI_ECONOMY=full` is max volume.  
**Consequences:** Lower YouTube draft caps, no vision on publish, no reasoning curators, no X search on sentiment, `minHoursBetweenRuns` gates. Revert with `XAI_ECONOMY=full` + runner restart + Worker deploy.
...
**Status:** accepted  
**Context:** Ben: switch CladFacts from economy mode to full and push to prod. Live had been economy since 2026-08-19.  
**Decision:** Run **full**. `XAI_ECONOMY=full` on the Mac runner (`runner/.env` + PM2 restart) and the production Worker (`wrangler.jsonc` vars). Staging stays economy / spend-dark (`deploy-staging.mjs` still forces `economy`). `XAI_ECONOMY=economy` is the throttle. Code default when unset remains full.  
**Consequences:** Full YouTube draft caps, vision on publish, reasoning curators, X search on sentiment, cron without `minHoursBetweenRuns`. Revert with `XAI_ECONOMY=economy` + runner restart + Worker deploy.
...
**Status:** accepted  
**Context:** Ben: put CladFacts back in economy mode. Production had been on full power since 2026-08-15.  
**Decision:** Run **economy**. `XAI_ECONOMY=economy` on the Mac runner (`runner/.env` + PM2 restart) and the Worker (`wrangler.jsonc` vars). `XAI_ECONOMY=full` is the throttle-off switch. Code default when unset remains full — the live env is what counts.  
**Consequences:** Lower YouTube draft caps, no vision on publish, no reasoning curators, no X search on sentiment, `minHoursBetweenRuns` gates. Revert with `XAI_ECONOMY=full` + runner restart + Worker deploy.
