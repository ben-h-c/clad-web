---
type: "broadcast"
headline: "NBC Nightly News reports on Delaware hospital shooting and foiled White House plot"
summary: "The broadcast opened with breaking news on a shooting at ChristianaCare's Wilmington Hospital in Delaware, where two people were shot and the campus was placed on lockdown. It then covered the FBI's foiling of an alleged plot targeting a White House UFC event involving explosive drones and snipers, with five suspects arrested across multiple states. Additional segments addressed Gulf Coast flooding, a potential first named storm, a fatal B-52 bomber crash killing eight, a police shooting of a one-year-old in Mississippi, updates on an Iran deal, the Washington Reflecting Pool turning green, and new details from the New York Times on Jeffrey Epstein's final days from cellmates."
publishedAt: 2026-06-17T02:05:29Z
sourceUrl: "https://www.youtube.com/watch?v=1Fjw9jABBpk"
sourceTitle: "NBC News"
section: "Politics"
letterGrade: "B+"
factualityScore: 85
politicalLean: "center"
leanScore: 0
leanRationale: "Standard broadcast framing with no discernible partisan slant in story selection or language"
gradeRationale: "Mostly accurate live reporting on confirmed events with minor missing context on developing investigations and victim details"
topics:
  - "Hospital shooting"
  - "White House security threat"
  - "Military aviation crash"
  - "Epstein death investigation"
assessment: "The program provided timely updates on multiple unfolding stories using on-scene reporters and official statements. Claims about the hospital shooting, UFC plot, and B-52 crash align with contemporaneous reporting from multiple outlets. The Epstein segment accurately summarized the NYT investigation's focus on prior suicide attempts and ignored warnings. Minor limitations include lack of updates on victim conditions in active cases and reliance on anonymous officials for some details. Framing was straightforward without sensationalism or omission of key counterpoints in developing probes."
notableConcerns: []
keyMoments:
  - claim: "Two people shot at Wilmington Hospital; campus on lockdown with active shooter search"
    verdict: "verified"
    note: "Confirmed by local police and hospital statements; suspect later taken into custody"
  - claim: "FBI arrested five suspects in plot to attack White House UFC event using explosive drones and snipers"
    verdict: "verified"
    note: "Court documents and FBI statements corroborate arrests and alleged plans involving TikTok coordination"
  - claim: "B-52 bomber crash killed eight crew members shortly after takeoff"
    verdict: "verified"
    note: "Air Force confirmed crash at Edwards AFB on June 15 with all aboard fatalities"
  - claim: "NYT reporting from Epstein cellmates details prior suicide attempts and ignored warnings"
    verdict: "verified"
    note: "NYT investigation published June 16 cites cellmates and documents pointing to suicide"
videoId: "1Fjw9jABBpk"
videoTitle: "Delaware hospital shooting, FBI foils White House plot | NBC Nightly News with Tom Llamas - June 16"
thumbnail: "https://img.youtube.com/vi/1Fjw9jABBpk/maxresdefault.jpg"
citations:
  - title: "2 workers reportedly shot inside Wilmington Hospital in Delaware; facility on lockdown"
    url: "https://6abc.com/post/possible-shooting-reported-wilmington-hospital-delaware/19310822/"
  - title: "FBI foils explosive-drone, sniper-fire terror plot targeting White House UFC fight"
    url: "https://www.timesofisrael.com/fbi-foils-explosive-drone-sniper-fire-terror-plot-targeting-white-house-ufc-fight/"
  - title: "8 crew members killed in B-52 bomber crash at California’s Edwards Air Force Base"
    url: "https://www.cnn.com/2026/06/15/us/b-52-crash-edwards-california"
  - title: "The Untold Story of Jeffrey Epstein's Death and His Final Days"
    url: "https://www.nytimes.com/interactive/2026/06/16/magazine/jeffrey-epstein-death-final-days.html"
---


