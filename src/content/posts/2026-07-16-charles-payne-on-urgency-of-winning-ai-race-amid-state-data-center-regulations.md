---
type: "broadcast"
headline: "Charles Payne on urgency of winning AI race amid state data center regulations"
summary: "The segment features Charles Payne arguing the U.S. must win the AI race without pause, criticizing New York Gov. Hochul's approach to AI data centers and linking Democratic climate policies to energy constraints. It highlights electricity price differences and data center growth across states like New York, Texas, Virginia, and Arizona, claiming high prices in regulated states deter investment. Payne frames opposition as politically motivated by climate priorities and anti-billionaire sentiment."
publishedAt: 2026-07-16T23:30:38.000Z
sourceUrl: "https://www.youtube.com/watch?v=W1XkBXuNCN8"
sourceTitle: "Fox Business"
section: "Politics"
letterGrade: "B-"
factualityScore: 65
politicalLean: "right"
leanScore: 72
leanRationale: "Strong right-leaning framing via criticism of 'socialist Democrats,' climate agenda, and nuclear closures; emphasizes deregulation and AI competition while downplaying grid or environmental tradeoffs."
gradeRationale: "Graded B-: core data center counts, prices, and Hochul pause are directionally accurate per 2026 sources, but selective framing, outdated NY facility count, and uncontextualized price causation claims reduce reliability."
topics:
  - "AI & Tech"
  - "electricity prices"
  - "New York energy policy"
assessment: "The broadcast accurately captures Hochul's recent one-year pause on large data centers and notes higher NY electricity rates alongside fewer facilities. However, it presents price and load data without noting studies showing weak or inverse correlation between data center growth and retail prices, or that data centers often secure lower industrial rates. Nuclear plant closures predate the AI boom but contribute to NY constraints; broader context on grid strain in high-growth states like Virginia is omitted. Framing leans partisan by tying policies to 'socialist Democrats' and China collaboration without evidence."
notableConcerns:
  - "Outdated or narrow count of NY data centers (sources show ~155 total facilities)"
  - "Implied direct causation between regulation and prices/load without supporting analysis"
  - "Omission of data center-driven wholesale price spikes and grid impacts in pro-growth states"
keyMoments:
  - claim: "NY has only 9 data centers while Texas has 125, Virginia 371+"
    verdict: "missing context"
    note: "Recent 2026 data show NY ~155 facilities (4.5 TWh) vs. VA ~665 (24 TWh), TX ~413 (17 TWh); transcript likely refers to hyperscale only."
  - claim: "NY electricity at 30 cents/kWh vs. TX/VA 17 cents, AZ 14 cents"
    verdict: "verified"
    note: "NY residential rates among highest nationally (~29 cents); TX/VA/AZ lower on average per EIA-linked reports."
  - claim: "Hochul pushing pause on AI for political/climate reasons"
    verdict: "verified"
    note: "July 2026 executive order imposes one-year moratorium on hyperscale data centers citing energy grid and environment."
  - claim: "States closing nuclear plants hurt AI readiness; prices skyrocketed in CA/NY/ME"
    verdict: "missing context"
    note: "Indian Point closure contributed to NY issues; analyses (LBNL, IER) find data centers not primary driver of most price rises 2019-2025."
videoId: "W1XkBXuNCN8"
videoTitle: "We have 'NO TIME' to waste, we MUST win the AI race: Charles Payne"
thumbnail: "https://img.youtube.com/vi/W1XkBXuNCN8/maxresdefault.jpg"
mediaNote: "default overlay framing"
thumbFocusY: 32
thumbFocusX: 50
mediaStyle: "overlay"
citations:
  - title: "U.S. Data Center Power Consumption Map by State (2026)"
    url: "https://www.electricchoice.com/datacenters/"
  - title: "US data centers' energy use amid the artificial intelligence boom"
    url: "https://www.pewresearch.org/short-reads/2025/10/24/what-we-know-about-energy-use-at-us-data-centers-amid-the-ai-boom/"
  - title: "Data Centers and Their Energy Consumption"
    url: "https://www.congress.gov/crs-product/R48646"
  - title: "Have Data Centers Driven Up Electricity Prices? The State-Level Data Don't Support the Narrative"
    url: "https://www.instituteforenergyresearch.org/the-grid/have-data-centers-driven-up-electricity-prices-the-state-level-data-dont-support-the-narrative/"
  - title: "Governor Hochul Signs Nation-Leading Legislation to Require AI Frameworks"
    url: "https://www.governor.ny.gov/news/governor-hochul-signs-nation-leading-legislation-require-ai-frameworks-ai-frontier-models"
  - title: "Gov. Kathy Hochul hits pause on AI data centers"
    url: "https://www.youtube.com/watch?v=B4Ls62hFxC4"
  - title: "AI, Data Centers, and the U.S. Electric Grid: A Watershed Moment"
    url: "https://www.belfercenter.org/research-analysis/ai-data-centers-us-electric-grid"
---


