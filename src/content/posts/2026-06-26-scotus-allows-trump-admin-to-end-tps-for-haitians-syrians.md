---
type: "broadcast"
headline: "SCOTUS Allows Trump Admin to End TPS for Haitians, Syrians"
summary: "Forbes Breaking News segment discusses a June 2026 6-3 Supreme Court ruling allowing the Trump administration to terminate Temporary Protected Status for Haitian and Syrian nationals. The interview with immigration attorney Shannon Legair Mangret explains the decision's bar on judicial review of DHS Secretary determinations, lower-court blocks that were lifted, and the statutory framework of TPS. It covers benefits under TPS, historical designations since 2010 for Haiti and 2012 for Syria, and potential next steps for holders including family petitions or asylum claims."
publishedAt: 2026-06-26T19:48:40.000Z
sourceUrl: "https://www.youtube.com/watch?v=Fyi7LypBOak"
sourceTitle: "Forbes Breaking News"
section: "Politics"
letterGrade: "B+"
factualityScore: 82
politicalLean: "center"
leanScore: -18
leanRationale: "Emphasized humanitarian impacts, dissent's racial-animosity arguments, and economic contributions while downplaying administration's national-interest rationale and prior extensions under multiple presidents."
gradeRationale: "Graded B+: core legal ruling, 6-3 vote, Alito opinion, Kagan dissent, and procedural bar on judicial review accurately reported; minor numerical inconsistencies on TPS counts and slight overstatement of immediate nationwide effects."
topics:
  - "TPS"
  - "Supreme Court"
  - "Haiti"
  - "Syria"
assessment: "The segment accurately conveys the holding that TPS termination decisions are generally unreviewable except for constitutional claims, which the Court viewed as unlikely to succeed. It correctly notes the dissent's focus on procedural adequacy and cited statements. Viewers may miss that TPS is statutorily temporary, has been extended or terminated across administrations, and that the ruling preserves constitutional challenges while clarifying congressional intent on review. The guest's advocacy framing highlights potential economic disruption but omits that many holders have other relief avenues or that conditions in origin countries are assessed by DHS with interagency input."
notableConcerns: []
keyMoments:
  - claim: "SCOTUS 6-3 ruled TPS designation decisions not subject to judicial review"
    verdict: "verified"
    note: "Matches Alito opinion in Mullin v. Doe (SCOTUSblog, SCOTUS.gov opinion 25-1083)."
  - claim: "Approximately 350,000 Haitians and 6,000 Syrians affected"
    verdict: "verified"
    note: "Consistent with contemporaneous reporting; minor transcript variance on Syrian figure does not alter substance."
  - claim: "Kagan dissent highlighted Trump's comments and racial undertones"
    verdict: "verified"
    note: "Dissent references statements and argues they fairly shout racial overtones (SCOTUS opinion)."
  - claim: "Lower courts in DC and NY had blocked terminations pending review"
    verdict: "verified"
    note: "Confirmed by SCOTUSblog and multiple outlets; Court vacated those stays."
videoId: "Fyi7LypBOak"
videoTitle: "This Is What Happens Next To Haitian & Syrian TPS Holders After Trump's SCOTUS Victory"
thumbnail: "https://img.youtube.com/vi/Fyi7LypBOak/maxresdefault.jpg"
mediaNote: "16:9 band default"
thumbFocusY: 40
thumbFocusX: 50
mediaStyle: "overlay"
citations:
  - title: "Court allows Trump administration to end removal protections for Syrian and Haitian nationals"
    url: "https://www.scotusblog.com/2026/06/supreme-court-allows-trump-administration-to-end-removal-protections-for-syrian-and-haitian-nati/"
  - title: "25-1083 Mullin v. Doe (06/25/2026)"
    url: "https://www.supremecourt.gov/opinions/25pdf/25-1083_f204.pdf"
  - title: "Supreme Court allows Trump to end protected status for Haitian and Syrian migrants"
    url: "https://www.bbc.com/news/articles/c4g8ym422lko"
  - title: "Supreme Court lets Trump end deportation protections for Syrians and Haitians"
    url: "https://www.npr.org/2026/06/25/nx-s1-5844292/supreme-court-syrian-haitian-tps"
  - title: "Supreme Court allows Trump to remove protections from thousands of Haitian and Syrian immigrants"
    url: "https://www.nbcnews.com/politics/supreme-court/supreme-court-allows-trump-remove-protections-thousands-haitian-syrian-rcna263164"
  - title: "Temporary Protected Status (TPS): Fact Sheet"
    url: "https://forumtogether.org/article/temporary-protected-status-fact-sheet/"
---


