---
type: "broadcast"
headline: "Trump Comments on Pending US-Iran Deal Signing"
summary: "The segment features President Trump responding to a question about the status of the pending US-Iran agreement and whether a signing would occur on Friday. Trump describes his experience with deals, notes uncertainty, expresses optimism that parties want to sign, and clarifies it is a detailed memorandum of understanding rather than a final contract.\nThe clip draws from a press event tied to G7 discussions, with Trump speaking directly; no additional guests or graphics are referenced in the available transcript."
publishedAt: 2026-06-17T16:47:28Z
sourceUrl: "https://www.youtube.com/watch?v=JhR9FND8N8A"
sourceTitle: "Associated Press"
section: "Politics"
letterGrade: "B+"
factualityScore: 85
politicalLean: "center"
leanScore: 0
leanRationale: "Neutral factual reporting from AP with no discernible political slant in framing or word choice."
gradeRationale: "Graded B+: claims about deal status and signing timeline accurately reflect public statements and reporting, with minor issues from possible transcription error on deal name and limited sourcing details."
topics:
  - "Iran"
  - "US-Iran deal"
  - "Middle East"
assessment: "The broadcast accurately captures Trump's cautious yet optimistic remarks on the preliminary US-Iran MoU amid ongoing 2026 negotiations to end hostilities and reopen the Strait of Hormuz. Reporting aligns with multiple outlets confirming the Friday signing target in Switzerland, though the deal remains non-final with terms largely undisclosed. Viewers may miss context on the preceding conflict, sanctions relief elements, and Iran's expressed reservations on timing. Framing is straightforward without loaded language or selective emphasis."
notableConcerns: []
keyMoments:
  - claim: "The deal is a detailed memorandum of understanding, not a short or final contract."
    verdict: "verified"
    note: "Matches descriptions in contemporaneous reporting from Reuters, CNN, and Axios of the preliminary MoU."
  - claim: "Signing could happen Friday but 'you never know' as deals can fall through."
    verdict: "verified"
    note: "Directly aligns with Trump's public statements and reporting on the scheduled June 19 ceremony amid finalization uncertainties."
  - claim: "Parties want to sign and return to normal life."
    verdict: "verified"
    note: "Consistent with statements from US and mediating parties on mutual interest in concluding the agreement."
videoId: "JhR9FND8N8A"
videoTitle: "Trump on whether the Iran deal will be signed Friday: 'You never know'"
thumbnail: "https://img.youtube.com/vi/JhR9FND8N8A/maxresdefault.jpg"
citations:
  - title: "U.S. and Iran reach deal to end the Mideast war, with agreement set to be signed Friday"
    url: "https://www.cnbc.com/2026/06/14/us-iran-war-peace-deal.html"
  - title: "What's in the Iran deal Trump says he's ready to sign"
    url: "https://www.axios.com/2026/06/12/iran-deal-mou-strait-open-sanctions-relief"
  - title: "Trump says the US and Iran have signed a deal to end the war - as it happened"
    url: "https://www.reuters.com/world/iran-war-live-trump-says-us-tehran-have-reached-peace-deal-2026-06-15/"
  - title: "2025–2026 Iran–United States negotiations"
    url: "https://en.wikipedia.org/wiki/2025%E2%80%932026_Iran%E2%80%93United_States_negotiations"
---


