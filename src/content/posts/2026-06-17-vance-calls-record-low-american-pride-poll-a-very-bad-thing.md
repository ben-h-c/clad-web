---
type: "broadcast"
headline: "Vance Calls Record-Low American Pride Poll a 'Very Bad Thing'"
summary: "The segment on Gutfeld! discussed a recent poll showing American pride at or near record lows ahead of the country's 250th anniversary. JD Vance and panelists reacted to findings of 56% overall extremely or very proud to be American, with 90% of Republicans vs. much lower Democratic figures, plus commentary on the American dream being harder to obtain for many Democrats.\nVance argued it reflects a lack of gratitude among some Democrats for the country itself rather than policy disagreements. Panelists attributed smirking at patriotism to attention-seeking or cultural messaging in media and films, contrasting it with stronger pride in heartland areas and among older Americans."
publishedAt: 2026-06-17T04:30:16Z
sourceUrl: "https://www.youtube.com/watch?v=QJne9ixVxBk"
sourceTitle: "Fox News"
section: "Politics"
letterGrade: "B"
factualityScore: 75
politicalLean: "right"
leanScore: 65
leanRationale: "Emphasized Democratic lack of gratitude and patriotism while highlighting Republican pride; framed decline as cultural/elite-driven issue"
gradeRationale: "Poll numbers directionally accurate per Gallup (58% vs. stated 56%) showing record low pride with strong partisan gaps; minor discrepancies in exact figures and unverified Democratic 'American dream' statistic"
topics:
  - "American pride"
  - "Patriotism"
  - "Public opinion polls"
assessment: "The core poll claim aligns with Gallup's June 2025 data showing a record-low 58% extremely or very proud, driven largely by drops among Democrats. Minor variance in the exact percentage cited does not undermine the trend. Framing focuses on cultural and partisan explanations without exploring counter-data on policy dissatisfaction or historical context for pride fluctuations. Viewers miss broader polling on why pride varies, including economic concerns or generational shifts documented in the same surveys. The discussion is consistent with Fox's editorial perspective but accurately reports the directional findings."
notableConcerns: []
keyMoments:
  - claim: "56% of Americans extremely or very proud to be American, record low"
    verdict: "verified"
    note: "Gallup June 2025 poll found 58%; close match, confirmed record low since 2001"
  - claim: "90% of Republicans very proud"
    verdict: "verified"
    note: "Gallup showed ~92% of Republicans extremely or very proud"
  - claim: "Decline reflects lack of gratitude among Democrats for the country"
    verdict: "missing context"
    note: "Vance's interpretation; polls show partisan gaps but link pride to multiple factors including age and views on future"
videoId: "QJne9ixVxBk"
videoTitle: "JD Vance: This is a 'very, very bad thing'"
thumbnail: "https://img.youtube.com/vi/QJne9ixVxBk/maxresdefault.jpg"
mediaNote: "16:9 band default"
thumbFocusY: 40
thumbFocusX: 50
mediaStyle: "overlay"
citations:
  - title: "American Pride Slips to New Low"
    url: "https://news.gallup.com/poll/692150/american-pride-slips-new-low.aspx"
  - title: "Poll: America at 250 is riven with doubt and pessimism"
    url: "https://www.nbcnews.com/politics/politics-news/poll-america-250-riven-doubt-pessimism-glimmers-hope-rcna348912"
  - title: "Americans' pride in their country is declining, new Gallup poll shows"
    url: "https://www.foxnews.com/media/americans-pride-country-declining-new-gallup-poll-shows"
---


