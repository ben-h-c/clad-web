...
// Public share card — no gated averages (report counts only).
...
function markup(names: string[], n: number): string {
  ...
  return `<div style="display:flex;flex-direction:column;width:1200px;height:630px;background:${PAPER};color:${INK};font-family:Playfair;padding:48px 64px;border:14px solid ${INK}">
    ...
    <div style="display:flex;font-size:22px;letter-spacing:4px;color:${MUTED}">MARCH MADNESS FOR THE TRAIL</div>
    <div style="display:flex;font-size:58px;font-weight:700;line-height:1.08;margin-top:14px">Coverage Bracket</div>
    <div style="display:flex;font-size:30px;margin-top:22px;line-height:1.35">Top ${n} figures by graded coverage</div>
    <div style="display:flex;font-size:24px;color:${MUTED};margin-top:14px;line-height:1.4;max-width:1000px">${line || "Filling as reports publish"}</div>
    <div style="display:flex;margin-top:auto;justify-content:space-between;align-items:flex-end;width:100%">
      <div style="display:flex;border:2px solid ${RED};color:${RED};padding:10px 22px;font-size:22px;letter-spacing:2px">NOT POLLS — COVERAGE GRADES</div>
...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  // Guests' public card: no score reveal
  const board = buildCoverageBracket(all, false);
  const names = board.field.map((p) => p.name);

  const fonts = await loadFonts(new URL(request.url).origin);
  const img = new ImageResponse(markup(names, board.field.length), {
    width: 1200,
    height: 630,
    fonts: fonts as any,
    format: "png",
