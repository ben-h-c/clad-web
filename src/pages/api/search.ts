...
// Server-side search: scores + filters posts on the server and returns only the
// matching results, so the browser never downloads the full index. Grades + lean
// are omitted for restricted (free/anonymous) readers — same model as the rest
// of the site.
export const GET: APIRoute = async ({ request }) => {
  ...
  const scored: { p: (typeof posts)[number]; s: number }[] = [];
